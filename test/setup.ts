...
// 1. tools/rnw-preload.ts registers happy-dom and resolves `react-native` to
//    `react-native-web` (with the optional native peers stubbed), so the kit's modules load
//    under `bun test` and tests render the exact web output.
// 2. The viewport stub and the per-test hooks below.
import "../tools/rnw-preload.ts";
