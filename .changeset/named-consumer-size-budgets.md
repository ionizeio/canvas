...
Gate named-import distribution sizes for Button, Input, DataTable, and StackedList alongside the existing whole-kit budget, independently for web, iOS and Android. Use pinned esbuild with supported export conditions and platform extension ordering, verify those choices with a real package fixture, preserve component exports, and fail on missing or invalid output. Required and optional peers are externalized; these measurements budget Canvas distribution code, not complete native application sizes. Stock Metro package validation remains separate.
