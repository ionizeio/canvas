...
Bespoke art (a galactic core, a comet, a lattice of cells) goes in a `Backdrop.Custom` layer and binds to the engine's clock through `LoopView`. `backdropClock(energy)` hands out the shared channels (`flight`, `twinkle`, `scintillate`, `drift`, `breath`, `event`), and a `LoopView` maps one onto its opacity or a transform component with an `inputRange` / `outputRange` pair plus a phase `offset`, so the custom layer stays in phase with the particle layers and costs the same nothing per frame: the native driver on iOS and Android, a compositor-run CSS animation on the web. A custom layer lays its art out against the surface's own box, which `useBackdropBox()` returns (width, height and the scene's focus point; null outside a surface), so the same scene fills a documentation stage or a card as exactly as the screen.
