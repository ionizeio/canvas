...
// layered `:focus-visible` rule (styles/tokens/base.css), and an outline of width 0 paints
// nothing whatever its style or colour. The style is `solid`, not `none`, because React
// Native parses the outline keys too, on View and TextInput alike: its prop parser
// accepts only `solid`, `dotted` and `dashed`, and logs "Could not parse OutlineStyle"
// for anything else. Natively the zero width draws no outline either. React Native's own
// style types declare these keys, so they type-check without a cast and reject `none`.
//
