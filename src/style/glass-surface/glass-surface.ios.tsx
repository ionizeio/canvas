...
import {
  GlassBox, contrastBorder, specularRim, SHEER_FILL_OPACITY, materialFill,
...
export function GlassSurface(props: GlassSurfaceProps) {
  ...
  const { style, layer = "functional", brand, tint, interactive = false } = props;
  ...
  const material = solid ? null : <>
    {!native || brand == null || tint != null ? <View style={[fill, { backgroundColor: surfaceUnderFill(theme.glass, layer, brand, tint, theme.tokens.background), opacity: translucent ? SHEER_FILL_OPACITY : 1 }]} /> : null}
    {native && LiquidView ? <LiquidView glassEffectStyle="regular" isInteractive={interactive} tintColor={brand} colorScheme={theme.dark ? "dark" : "light"} style={fill} />
      : FrostView ? <FrostView intensity={surfaceIntensity(layer, translucent)} tint={theme.dark ? "dark" : "light"} style={fill} /> : null}
