 ...
 *      The same holds for the typefaces: docs/scripts/subset-fonts.mjs cuts the Urbanist
 ...
 *      with every copy, modified ones included.
 *
...
const bakedData = ["@expo-google-fonts/geist-mono", "@expo-google-fonts/urbanist", "lucide-static"];
...
writeFileSync(
  ...
  `${JSON.stringify(
    {
      _comment:
        "GENERATED by tools/noticegen/scan.ts (bun run notices:scan). The packages the docs app actually ships: the union of what survives into the web, iOS and Android JS bundles (read from source-mapped exports), what autolinks native code (an expo-module.config.json, or the classic react-native-config autolinking that links podspec/gradle modules and react-native itself), and the data the generators bake in (the lucide-static icon geometry, the subset Urbanist and Geist Mono faces). Re-run after changing the app's dependencies, then run notices:gen.",
      counts: {
